...
export default function MitraAI({ onNavigate }: { onNavigate: (tabId: string) => void }) {
  ...
  React.useEffect(() => {
    ...
    const isValid = key && key !== '' && key !== 'undefined' && !key.includes('MY_GEMINI_API_KEY');
    
    if (isValid && !aiRef.current) {
  ...
  const handleSend = async () => {
    ...
    if (!hasApiKey || !aiRef.current) {
      toast.error('Mitra requires an API Key to process your request.');
      setIsOpen(true);
      return;
    }
                ...
                <div className="flex items-center gap-4 relative">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className={cn("w-1.5 h-1.5 rounded-full animate-ping", hasApiKey ? "bg-green-400" : "bg-red-400")} />
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">
                        {hasApiKey ? "Syncing..." : "Auth Required"}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  {!hasApiKey && (
                    <div className="p-6 rounded-3xl bg-red-500/10 border border-red-500/20 space-y-4">
                      <div className="flex items-center gap-3 text-red-600">
                        ...
                        <h4 className="font-black text-sm uppercase tracking-wider">Neural Link Failed</h4>
                      </div>
                      <p className="text-xs font-semibold text-slate-600 leading-relaxed">
                        Mitra requires a Gemini API Key to initialize its neural core. Please configure the "GEMINI_API_KEY" secret in the project settings.
                      </p>
                      <Button 
                        variant="outline" 
                        size="sm" 
                      ...
                      >
                        <ExternalLink size={14} className="mr-2" /> Get API Key
                      </Button>
                    </div>
                  )}
              ...
              <div className="p-6 pt-2 border-t border-border/50">
                <div className="relative group">
                  <Input
                    ...
                    placeholder={hasApiKey ? "How can I assist you?" : "Configure API key to chat..."}
                    ...
                    disabled={!hasApiKey && messages.length > 1}
                  />
                  <button
                    onClick={handleSend}
                    disabled={!input.trim()}
